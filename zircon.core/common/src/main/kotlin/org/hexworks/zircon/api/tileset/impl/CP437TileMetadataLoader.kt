...
class CP437TileMetadataLoader(private val width: Int,
    ...
    private val cp437Metadata: Map<Char, CP437TextureMetadata> = unicodeToCP437Lookup.map { (unicodeChar, index) ->
        val x = index.rem(16)
        val y = index.div(16)
        val c = unicodeChar.toChar()
        Pair(c, CP437TextureMetadata(
                x = x,
                y = y,
                width = width,
                height = height,
    ...
    private val emptyTextureMeta: TextureMetadata<CharacterTile> = cp437Metadata[' ']!!
    ...
    override fun fetchMetaForTile(tile: CharacterTile): TextureMetadata<CharacterTile> {
